...
(function () {
  ...
  /** Prefer *.short i18n key; fall back to id when full label is long (bar Y-axis). */
  function tissueAxisLabel(id) {
    const shortKey = `omics.tissue.${id}.short`;
    const short = t(shortKey, '');
    if (short && short !== shortKey) return short;
    const full = tissueLabel(id);
    return full.length > 16 ? id : full;
  }

  function barChartHeight(count) {
    return Math.max(260, count * 22 + 56);
  }

  function axisLabelWidth(labels) {
    const maxChars = labels.reduce((m, l) => Math.max(m, String(l).length), 0);
    return Math.min(148, Math.max(64, maxChars * 6.2));
  }
  ...
  function syncPinnedUI(tissue) {
    const carouselBtn = document.querySelector(
      '.omics-mode-capsule .capsule-item[data-mode="carousel"]',
    );
    if (!carouselBtn) return;
    const pinLabel = carouselBtn.querySelector('.capsule-pin-label');
    if (tissue) {
      carouselBtn.classList.add('is-pinned');
      const name = tissueLabel(tissue);
      if (window.PervTip) {
        PervTip.set(carouselBtn, `${name} · ${t('home.omics.capsule.resume', 'click to resume carousel')}`);
      } else {
        carouselBtn.dataset.tip = `${name} · ${t('home.omics.capsule.resume', 'click to resume carousel')}`;
      }
      if (pinLabel) pinLabel.textContent = `· ${name}`;
    } else {
      carouselBtn.classList.remove('is-pinned');
      if (window.PervTip) PervTip.clear(carouselBtn);
      else {
        delete carouselBtn.dataset.tip;
        delete carouselBtn.dataset.i18nTip;
      }
  ...
  function bindCardInteractions() {
    document.querySelectorAll('.omics-tissue-card').forEach((card) => {
      ...
      card.addEventListener('click', () => {
        pinnedTissue = tissue;
        // Visually switch capsule to "Carousel" so clicking "Overview" can
        // properly clear the pin and return to the global overview.
        viewMode = 'carousel';
        updateCapsuleUI('carousel');
        stopCarousel();
        syncPinnedUI(tissue);
        syncCharts(true);
      });
  ...
  function bindCapsuleToggle() {
    ...
    capsule.querySelectorAll('.capsule-item').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        // Skip only when the mode is already active AND no tissue is pinned;
        // if a tissue is pinned, always respond so clicking Overview clears it.
        if (mode === viewMode && !pinnedTissue) return;
        pinnedTissue = null;
  ...
  function setViewMode(mode, animate) {
    ...
    // Always clear pinned UI when switching modes via the capsule.
    syncPinnedUI(null);
    if (mode === 'overview') {
      stopCarousel();
      clearCardHighlight();
  ...
  function entriesToPieData(entries, colorFn, labelFn, displayLabelFn) {
    const CA = window.ChartAnim;
    return entries.map(([name, value]) => {
      const fullLabel = labelFn ? labelFn(name) : name;
      const displayLabel = displayLabelFn ? displayLabelFn(name) : fullLabel;
      const color = colorFn(name);
      if (CA && CA.pieDataItem) {
        const item = CA.pieDataItem(displayLabel, value, color);
        if (fullLabel !== displayLabel) item.fullName = fullLabel;
        return item;
      }
      const item = {
        name: displayLabel,
  ...
  function pieOption(data, title, opts) {
    ...
    const hideLabels = o.cellLinePie && data.length <= 1;
    const showLabels = !many && !hideLabels;
    // Pie on the left ~58% of width; vertical legend on the right ~42%.
    const pieCenterX = '38%';
    const pieCenterY = title ? '52%' : '50%';
    return {
      animation: true,
      animationDuration: 1100,
      animationDurationUpdate: 750,
      ...
      tooltip: {
        trigger: 'item',
        formatter(p) {
          const full = (p.data && p.data.fullName) || p.name;
          return `${full}: ${p.value} (${p.percent}%)`;
        },
      },
      legend: {
        type: 'scroll',
      ...
      series: [Object.assign({}, pieSeriesAnim(), {
        ...
        labelLine: { show: showLabels, length: 8, length2: 6 },
        emphasis: {
          scale: true,
          scaleSize: 6,
          label: {
            ...
            formatter: (params) => `{name|${params.name}}\n{pct|${params.percent}%}`,
            rich: {
              name: { fontSize: 10, color: '#1e293b', lineHeight: 17, align: 'center' },
              pct: { fontSize: 10, color: '#475569', lineHeight: 15, align: 'center', padding: [4, 0, 0, 0] },
            },
          },
        },
        data,
  ...
  function barOption(entries, colorFn, labelFn, title, horizontal, preserveOrder, opts) {
    const o = opts || {};
    const axisLabelFn = o.axisLabelFn || labelFn;
    const sorted = preserveOrder ? [...entries] : sortByCountDesc([...entries]);
    const fullLabels = sorted.map(([n]) => (labelFn ? labelFn(n) : n));
    const axisLabels = sorted.map(([n]) => (axisLabelFn ? axisLabelFn(n) : n));
    ...
    if (horizontal) {
      const yLabels = [...axisLabels].reverse();
      const yFullLabels = [...fullLabels].reverse();
      ...
      return Object.assign({}, barAnim, {
        ...
        tooltip: {
          ...
          formatter(params) {
            const p = params[0];
            const full = yFullLabels[p.dataIndex] || p.name;
            return `${full}<br/>${p.marker}${p.value}`;
          },
        },
        grid: { left: 6, right: 28, top: title ? 26 : 10, bottom: 24, containLabel: true },
        xAxis: {
          type: 'value',
          min: 0,
          max: maxV + Math.max(0.5, maxV * 0.35),
          minInterval: 1,
          axisLabel: { margin: 10, fontSize: 10, color: '#64748b' },
    ...
    return Object.assign({}, barAnim, {
      ...
      grid: { left: 8, right: 8, top: title ? 28 : 10, bottom: 52, containLabel: true },
      xAxis: {
        type: 'category',
        data: fullLabels,
        axisLabel: {
          fontSize: 10,
          rotate: fullLabels.length > 6 ? 35 : 0,
          color: '#475569',
          margin: 12,
        },
      },
  ...
  function renderPanels(config, animate, onComplete) {
    ...
    // Freeze the section height before the destructive rebuild so emptying the
    // grid / hiding the matrix panel can't collapse the page (which would make
    // the browser clamp the scroll position upward when viewing near the bottom).
    const anchorY = window.scrollY || window.pageYOffset || 0;
    const lockedH = body ? body.offsetHeight : 0;
    if (body && lockedH) body.style.minHeight = lockedH + 'px';
    ...
    const releaseHeight = () => {
      ...
      // Re-pin to the captured position so a net height shrink can't make the
      // browser clamp the scroll upward. Skip if the user deliberately scrolled
      // further down during the transition (don't fight them).
      const curY = window.scrollY || window.pageYOffset || 0;
      if (curY <= anchorY + 1) {
        const doc = document.documentElement;
        const maxY = Math.max(0, (doc ? doc.scrollHeight : 0) - window.innerHeight);
        const targetY = Math.min(anchorY, maxY);
        if (Math.abs(curY - targetY) > 1) window.scrollTo(0, targetY);
      }
    };
    ...
    const doRender = () => {
      ...
      config.forEach((cfg, idx) => {
        ...
        const isHoriz = cfg.horizontal !== false;
        if (barEl && isHoriz && cfg.entries.length) {
          barEl.style.height = `${barChartHeight(cfg.entries.length)}px`;
        } else if (barEl) {
          barEl.style.height = '';
        }
        const pieData = entriesToPieData(
          cfg.entries, cfg.colorFn, cfg.labelFn, cfg.pieLabelFn,
        );
        const pieTitle = t('home.omics.charts.pie', 'Distribution');
        const barTitle = t('home.omics.charts.bar', 'File counts');
        mountChart(pieEl, () => pieOption(pieData, pieTitle, {
          compact: cfg.compactPie,
          cellLinePie: cfg.cellLinePie,
        }));
        mountChart(barEl, () => barOption(
          cfg.entries, cfg.colorFn, cfg.labelFn, barTitle,
          isHoriz, cfg.preserveOrder === true,
          { axisLabelFn: cfg.axisLabelFn },
        ));
      });
  ...
  function renderGlobalCharts(animate) {
    ...
    renderPanels([
      ...
      },
      {
        ...
        labelFn: tissueLabel,
        pieLabelFn: tissueAxisLabel,
        axisLabelFn: tissueAxisLabel,
        compactPie: true,
      },
      {
        key: 'assay',
  ...
  function renderTissueCharts(tissue, animate) {
    ...
    const isCellLine = !!(td && td.is_cell_line);
    const panel = document.getElementById('omics-tissue-panel');
  ...
  function renderMatrix(td, wrapId) {
    ...
    corner.textContent = t('home.omics.modal.col_period', 'Stage \\ Sequencing target');
