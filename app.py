...
META_BY_SEQ: dict[str, dict[str, Any]] = {
    r["Sequence.ID"]: r for r in META if r.get("Sequence.ID")
}
ABBR_GROUP: dict[str, str] = {}
for _row in META:
    _abbr = _row.get("Abbretiation", "")
    if _abbr and _abbr not in ABBR_GROUP:
        ABBR_GROUP[_abbr] = _row.get("Group", "") or ""
...
# ---------------------------------------------------------------------------
# BLAST databases: build a nucleotide DB from the 1165 intact PERVs and a
# protein DB from the annotated ORFs (Gag/Pol/Env ...). Indexes are rebuilt
# only when the source FASTA / annotation is newer (mirrors build_indexes).
# ---------------------------------------------------------------------------

from server import blast_engine as _blast  # noqa: E402


def _build_perv_protein_fasta(out_path: Path) -> int:
    """Translate annotated ORFs of every sequence into a protein multi-FASTA.

    Sequence ids are ``{sid}__{orf_name}`` (pipe-free so BLAST parses them as
    plain accessions), enabling the front-end to recover the PERV id by
    splitting on ``__``. Returns the number of protein records written.
    """
    chunks: list[str] = []
    for sid, regions in ORF_INDEX.items():
        if sid not in SEQ_OFFSETS:
            continue
        full = read_fasta_full(PASS_FA_PATH, SEQ_OFFSETS[sid])
        for r in regions:
            if r.get("name") == "LTR":
                continue
            ...
            if not protein:
                continue
            seqid = _blast.sanitize_seqid(f"{sid}__{r.get('name', 'orf')}")
            chunks.append(f">{seqid}\n{fasta_wrap(protein)}\n")
    out_path.write_text("".join(chunks), encoding="utf-8")
    return len(chunks)


def _run_makeblastdb(in_fa: Path, stem: Path, dbtype: str, title: str) -> None:
    import subprocess

    if not in_fa.exists() or in_fa.stat().st_size == 0:
        print(f"[WARN] makeblastdb skipped, missing/empty input: {in_fa}")
        return
    cmd = [
        _blast.MAKEBLASTDB_BIN,
        "-in", str(in_fa),
        "-dbtype", dbtype,
        "-parse_seqids",
        "-out", str(stem),
    ...
    try:
        ...
        out = getattr(exc, "output", b"") or b""
        tail = out.decode("utf-8", errors="replace")[-400:] if out else ""
        print(f"[WARN] makeblastdb failed for {stem}: {exc}\n{tail}")


def build_blast_databases(force: bool = False) -> None:
    """Create / refresh the BLAST nucleotide and protein databases."""
    _blast.BLAST_DB_DIR.mkdir(parents=True, exist_ok=True)

    nt_stem = Path(_blast.DATABASES["perv_nt"]["stem"])
    nt_marker = Path(str(nt_stem) + ".nin")
    if force or _needs_rebuild(nt_marker, [ALL_FA_PATH]):
        _run_makeblastdb(ALL_FA_PATH, nt_stem, "nucl", "PERV intact nucleotide")

    prot_fa = _blast.BLAST_DB_DIR / "perv_protein.fa"
    if force or _needs_rebuild(prot_fa, [PASS_FA_PATH, ORF_JSON]):
        _build_perv_protein_fasta(prot_fa)
    prot_stem = Path(_blast.DATABASES["perv_protein"]["stem"])
    prot_marker = Path(str(prot_stem) + ".pin")
    if force or _needs_rebuild(prot_marker, [prot_fa]):
        _run_makeblastdb(prot_fa, prot_stem, "prot", "PERV ORF protein")


build_blast_databases(force=False)


# ---------------------------------------------------------------------------
# Background cache warm-up: pre-load expensive lazy caches so the first
# real HTTP request (e.g. homepage KPI fetch) returns immediately.
# ---------------------------------------------------------------------------

# Page-cache warm-up budget for recommended `.bw` tracks. We only read the
# head + tail of each file: bigwig keeps its header / chrom B-tree / zoom
# headers at the front and the zoom-level summary indexes near the end, which
# is exactly what igv.js touches first when rendering the initial zoomed-out
# view. Reading whole files would be far too much I/O for the slow shared disk.
_MO_WARM_MAX_FILES = 48
_MO_WARM_EDGE_BYTES = 4 * 1024 * 1024   # 4 MB from each end
_MO_WARM_SLEEP_S = 0.05                 # throttle so we don't storm shared IOPS


def _warm_multiomics_page_cache() -> None:
    """Pull the head/tail of recommended `.bw` files into the OS page cache.

    Runs in the startup daemon thread so the first user click on a popular
    track hits warm cache instead of a cold read from the slow shared disk.
    Best-effort: any error is swallowed so warm-up never breaks startup.
    """
    try:
        groups = _load_multiomics_recommended()
    except Exception:
        return
    ...
    for group in groups:
        for f in group.get("files", []):
            ...
            key = str(bw_path)
            if key in seen or not bw_path.is_file():
                continue
            seen.add(key)
            count += 1
            try:
                ...
                with bw_path.open("rb") as fh:
                    fh.read(_MO_WARM_EDGE_BYTES)
                    if size > 2 * _MO_WARM_EDGE_BYTES:
                        fh.seek(max(0, size - _MO_WARM_EDGE_BYTES))
                        fh.read(_MO_WARM_EDGE_BYTES)
            except Exception:
                pass
            time.sleep(_MO_WARM_SLEEP_S)


def _warm_caches() -> None:
    """Pre-populate lazy caches in a daemon thread at startup."""
    for _fn_name in ("_load_homologous", "_load_multiomics_meta",
                     "_load_multiomics_recommended"):
        try:
            fn = globals().get(_fn_name)
            if callable(fn):
                fn()
        except Exception:
            pass
    # Homepage Try pills derive from static data but their builder calls
    # url_for(), which needs a request context — push a temporary one so the
    # ``/`` route never pays the FASTA-scan + SQLite-lookup cost on first hit.
    try:
        builder = globals().get("_build_home_try_examples")
        if callable(builder):
            with app.test_request_context("/"):
                builder()
    except Exception:
        pass
    # Gene/transcript search index: build once here so the first user to hit
    # the search box doesn't pay the (~1-2s) full-table-scan cost themselves.
    try:
        if GENOME_DB.exists():
            _conn = genome_lib.gtf_connect(GENOME_DB)
            try:
                genome_lib.warm_search_index(_conn)
            finally:
                _conn.close()
    except Exception:
        pass
    # Done last: best-effort page-cache priming for popular `.bw` tracks. It is
    # the slowest step (cold reads from the shared disk) so it must not delay
    # the lighter caches above.
    try:
        _warm_multiomics_page_cache()
    except Exception:
        pass


import threading as _threading
_threading.Thread(target=_warm_caches, daemon=True, name="cache-warm").start()
...
# Static-asset cache busting: use per-file mtime so only actually changed
# files are refetched after a restart; fall back to process start time.
_ASSET_VERSION = str(int(time.time()))
_ASSET_MTIME_CACHE: dict[str, str] = {}


def _asset_url(filename: str) -> str:
    """Return a cache-busted URL for a static asset using its mtime."""
    v = _ASSET_MTIME_CACHE.get(filename)
    if v is None:
        try:
            p = os.path.join(app.static_folder, filename)
            v = str(int(os.path.getmtime(p)))
        except OSError:
            v = _ASSET_VERSION
        _ASSET_MTIME_CACHE[filename] = v
...
def _inject_asset_helpers():
    ...
    return {"asset_v": _ASSET_VERSION, "asset": asset}


# ---------------------------------------------------------------------------
# Response post-processing: long-lived caching for versioned static assets
# and on-the-fly gzip for text payloads (no extra dependency / proxy needed).
# ---------------------------------------------------------------------------

import gzip as _gzip

# Compress these content types when they are large enough to be worth it.
_GZIP_TYPES = (
...
@app.after_request
def _optimize_response(resp: Response) -> Response:
    ctype = (resp.content_type or "").split(";")[0].strip()
    is_static = request.path.startswith("/static/")

    # 1) Aggressive caching for /static/* — safe because every URL carries
    #    ?v=<file mtime>, so any edit changes the URL and busts the cache.
    try:
    ...
    # 2) Transparent gzip for compressible text responses (including static
    #    JS/CSS, which Flask serves with direct_passthrough=True — flip that
    #    off so we can read and compress the body on first download).
    try:
        if (
...
def _first_pass139_id() -> str | None:
    """Return the first sequence ID from pass.139.fa, or None."""
    if not PASS_FA_PATH.is_file():
        return None
    with PASS_FA_PATH.open(encoding="utf-8", errors="replace") as fh:
        for line in fh:
            line = line.strip()
            if line.startswith(">"):
                return line[1:].split()[0] or None
    return None


_TRY_LOCUS_PAD_BP = 20_000
_TRY_MO_TRACKS_MAX = 2
_TRY_MO_PREFERRED_ASSAYS = ("ATAC", "H3K4me3")
# Keep in sync with generate_and_plot.py SEQTYPE_ORDER (+ RNA/WGBS aliases).
_MULTIOMICS_ASSAY_ORDER = (
    "ATAC", "H3K27ac", "H3K4me1", "H3K4me3", "H3K36me3", "H3K9ac",
    "Pol2", "H3K27me3", "H3K9me3",
    "RNA", "RNA_Rep1", "RNA_Rep2",
    "WGBS", "WGBS_Rep1", "WGBS_Rep2",
    "CTCF",
)
_MULTIOMICS_ASSAY_RANK = {name: i for i, name in enumerate(_MULTIOMICS_ASSAY_ORDER)}


def _assay_sort_key(target: str) -> tuple[int, str]:
    t = (target or "").strip()
    return (_MULTIOMICS_ASSAY_RANK.get(t, len(_MULTIOMICS_ASSAY_ORDER)), t)


def _sort_multiomics_files(files: list[dict]) -> list[dict]:
    """Sort track dicts by canonical assay order, then filename."""
    return sorted(
        files,
        key=lambda f: (
            _assay_sort_key(str(f.get("target") or "")),
            str(f.get("filename") or f.get("name") or ""),
        ),
    )


def _pick_try_gene_row() -> dict | None:
    """Return the first matching gene row for homepage Try examples (ACTB, then TP53)."""
    genome_ok, _ = _genome_ready()
    if not genome_ok:
        return None
    for candidate in ("ACTB", "TP53"):
        try:
            conn = _get_gtf_conn()
            try:
                rows = genome_lib.search_features(conn, candidate, 1)
            finally:
                conn.close()
            if rows:
                return rows[0]
...
def _pick_try_homo_locus() -> dict | None:
    """Return the first homologous locus (188 loci) with valid coordinates."""
    try:
        _, loci_map = _load_homologous()
        if not loci_map:
            return None

        def _sort_key(locus: dict) -> int:
            lid = str(locus.get("locus_id") or "")
            try:
                return int(lid.split("_")[1]) if "_" in lid else 0
            except (ValueError, IndexError):
                return 0

        for locus in sorted(loci_map.values(), key=_sort_key):
            chrom = str(locus.get("chrom") or "")
...
_HOME_TRY_EXAMPLES_CACHE: list[dict[str, str]] | None = None


def _compute_home_try_examples() -> list[dict[str, str]]:
    """Build homepage Try pill links from live data (auto-picked examples).

    Display order: browser → gene window → gene → perv → homo locus → multi-omics.
    """
    browser_item: dict[str, str] | None = None
    perv_item: dict[str, str] | None = None
    locus_item: dict[str, str] | None = None
    gene_item: dict[str, str] | None = None
    homo_item: dict[str, str] | None = None
    mo_item: dict[str, str] | None = None

    browser_id = _first_pass139_id()
    if browser_id:
        browser_item = {
            "label": browser_id,
            "href": f"{url_for('page_browser')}?id={quote(browser_id)}",
            "title_key": "home.try.title.browser",
        }

    try:
        pervs = _load_perv_list()
        if pervs:
            name = pervs[0]["name"]
            perv_item = {
                "label": name,
                "href": f"{url_for('page_genome')}?perv={quote(name)}",
                "title_key": "home.try.title.perv",
            }
    except Exception:
        pass

    gene_row = _pick_try_gene_row()
    if gene_row:
        ...
        try:
            ...
            gstart = gend = 0
        gene_label = (
            str(gene_row.get("gene_name") or "")
            or str(gene_row.get("gene_id") or "")
            or "ACTB"
        )
        if chrom and gend >= gstart:
            loc_start = max(1, gstart - _TRY_LOCUS_PAD_BP)
            loc_end = gend + _TRY_LOCUS_PAD_BP
            loc = f"{chrom}:{loc_start}-{loc_end}"
            locus_item = {
                "label": loc,
                "href": f"{url_for('page_genome')}?loc={quote(loc)}",
                "title_key": "home.try.title.locus",
            }
        gene_item = {
            "label": gene_label,
            "href": f"{url_for('page_genome')}?q={quote(gene_label)}",
            "title_key": "home.try.title.gene",
        }

    homo_locus = _pick_try_homo_locus()
    if homo_locus:
        homo_item = {
            "label": str(homo_locus["locus_id"]),
            "href": (
                f"{url_for('page_genome')}?homo_locus="
                + quote(str(homo_locus["locus_id"]))
            ),
            "title_key": "home.try.title.homo_locus",
        }
    ...
    try:
        ...
        for g in groups:
            if g.get("id") == "P100_Liver":
                chosen = g
                break
        if not chosen and groups:
            chosen = min(groups, key=lambda g: len(g.get("files") or []))
        if chosen:
            gid = str(chosen["id"])
            files = chosen.get("files") or []
            by_assay: dict[str, dict] = {}
            for f in files:
                assay = str(f.get("target") or "")
                if assay and assay not in by_assay:
                    by_assay[assay] = f
            picked: list[dict] = []
            for assay in _TRY_MO_PREFERRED_ASSAYS:
                if assay in by_assay:
                    picked.append(by_assay[assay])
                if len(picked) >= _TRY_MO_TRACKS_MAX:
                    break
            if len(picked) < _TRY_MO_TRACKS_MAX:
                seen_assay = {str(f.get("target") or "") for f in picked}
                for f in _sort_multiomics_files(files):
                    assay = str(f.get("target") or "")
                    if assay in seen_assay:
                        continue
                    picked.append(f)
                    seen_assay.add(assay)
                    if len(picked) >= _TRY_MO_TRACKS_MAX:
                        break
            picked = _sort_multiomics_files(picked)
            if picked:
                fnames = [str(f.get("filename") or f.get("name") or "") for f in picked]
                fnames = [f for f in fnames if f]
                assay = str(picked[0].get("target") or "") if picked else ""
                label = f"{gid} {assay}".strip() if assay else gid
                mo_item = {
                    "label": label,
                    "href": (
                        f"{url_for('page_genome')}?mo_tracks="
                        + quote(",".join(fnames))
                    ),
                    "title_key": "home.try.title.mo_tracks",
                }
    except Exception:
        pass

    items: list[dict[str, str]] = []
    for part in (browser_item, locus_item, gene_item, perv_item, homo_item, mo_item):
        if part is not None:
            items.append(part)
    return items


def _build_home_try_examples() -> list[dict[str, str]]:
    """Cached accessor for homepage Try pill links.

    The underlying computation does disk I/O (FASTA scan) plus a SQLite gene
    lookup, which made the ``/`` route's TTFB noticeably slower than the other
    (compute-free) pages. The examples derive from static data, so we compute
    them once and reuse the result on every subsequent request.
    """
    global _HOME_TRY_EXAMPLES_CACHE
    if _HOME_TRY_EXAMPLES_CACHE is None:
        _HOME_TRY_EXAMPLES_CACHE = _compute_home_try_examples()
    return _HOME_TRY_EXAMPLES_CACHE


@app.route("/")
def page_index():
    # Non-blocking: only server-side render the Try pills when the cache is
    # already warm (populated by the startup warm-up thread or a prior hit).
    # If the cache is still cold — e.g. the very first request right after a
    # restart — we render with no pills and let the browser fetch them via
    # /api/home/try_examples, so the homepage HTML is always returned instantly
    # instead of paying the FASTA-scan + SQLite-lookup cost on the request
    # thread.
    return render_template(
        "index.html", try_examples=_HOME_TRY_EXAMPLES_CACHE or []
    )


@app.route("/api/home/try_examples")
def api_home_try_examples():
    """Homepage Try pills, computed lazily and cached (see warm-up thread)."""
    resp = jsonify({"examples": _build_home_try_examples()})
...
def _build_abbr_counts(
    rows: list[dict], *, include_all_breeds: bool = False
) -> list[dict]:
    """Per-breed counts with γ/β split and group label (for stacked bar charts)."""
    abbr_counter = Counter(r.get("Abbretiation", "") for r in rows)
    abbr_gamma: Counter = Counter()
    abbr_beta: Counter = Counter()
    for r in rows:
        abbr = r.get("Abbretiation", "")
        erv_type = r.get("ERV.type", "") or ""
        if erv_type.startswith("γ"):
            abbr_gamma[abbr] += 1
        elif erv_type.startswith("β"):
            abbr_beta[abbr] += 1
    if include_all_breeds:
        all_abbrs = set(GENOME_INFO.keys()) | set(abbr_counter.keys())
    else:
        all_abbrs = set(abbr_counter.keys())
    abbr_sorted = sorted(all_abbrs, key=lambda k: (-abbr_counter.get(k, 0), k))
    return [
        {
            "name": k,
            "count": abbr_counter.get(k, 0),
            "gamma_count": abbr_gamma.get(k, 0),
            "beta_count": abbr_beta.get(k, 0),
            "group": ABBR_GROUP.get(k, ""),
            "full_name": GENOME_INFO.get(k, {}).get("full_name", k),
            "assembly": GENOME_INFO.get(k, {}).get("assembly", ""),
        }
        for k in abbr_sorted
    ]
...
def api_overview_stats():
    ...
    resp = jsonify(
        {
            ...
            "abbr_counts": _build_abbr_counts(META, include_all_breeds=True),
    ...
    # Stats are derived from static metadata; cache so the home KPIs and the
    # overview page do not recompute/re-fetch on every navigation.
    resp.headers["Cache-Control"] = "public, max-age=86400"
    return resp
...
def api_overview_table():
    ...
    abbr = request.args.get("abbr", "").strip()
    ...
    if abbr:
        rows = [r for r in rows if r.get("Abbretiation") == abbr]
...
def api_sequences_pass():
    ...
    for sid in sorted(SEQ_OFFSETS.keys()):
        meta = META_BY_SEQ.get(sid, {})
        abbr = meta.get("Abbretiation", "") or ""
        items.append(
            {
                "id": sid,
                ...
                "sample": abbr,
                "sample_name": GENOME_INFO.get(abbr, {}).get("full_name", abbr),
                "erv_type": meta.get("ERV.type", "") or "",
            }
        )
    return jsonify({"total": len(items), "items": items})


@app.route("/api/sequences/pass/stats")
def api_sequences_pass_stats():
    """Breed / γβ distribution for the 139 pass.139 annotated sequences."""
    pass_ids = set(SEQ_OFFSETS.keys())
    rows = [r for r in META if r.get("Sequence.ID") in pass_ids]
    type_counter = Counter(r.get("ERV.type", "") for r in rows)
    group_counter = Counter(r.get("Group", "") for r in rows)
    return jsonify(
        {
            "total": len(rows),
            "type_counts": dict(type_counter),
            "group_counts": dict(group_counter),
            "abbr_counts": _build_abbr_counts(rows, include_all_breeds=True),
        }
    )
...
@app.route("/api/sequences/<sid>/all-dna")
def api_sequence_all_dna(sid: str):
    kind = request.args.get("kind", "orf").lower()
    if kind == "orf":
        regions = [r for r in ORF_INDEX.get(sid, []) if r["name"] != "LTR"]
    elif kind == "domain":
    ...
    for r in regions:
        ...
        if r.get("strand", "+") == "-":
            sub = reverse_complement(sub)
        chunks.append(
            f">{sid}|{r['name']}|{r['start']}-{r['end']}|{r.get('strand', '+')}\n"
            + fasta_wrap(sub)
            + "\n"
        )
        items.append(
...
def api_genome_region_gtf_export():
    """Return GTF features as a downloadable .gtf file.

    - If a `transcript_id` query param is given, export is scoped strictly to
      that single isoform (transcript/exon/CDS/UTR/codon lines only, no
      parent `gene` line and no sibling transcripts).
    - Else if a `gene_id` query param is given, export is scoped strictly to
      that gene's own features (by gene_id), so overlapping neighbor genes
      are never included.
    - Otherwise falls back to a plain region (chrom/start/end) overlap query.
    """
    _require_genome()
    transcript_id = (request.args.get("transcript_id") or "").strip()
    ...
    try:
        if transcript_id:
            rows = genome_lib.features_by_transcript_id(conn, transcript_id, limit=200000)
            if types:
                rows = [r for r in rows if r["type"] in types]
        elif gene_id:
            rows = genome_lib.features_by_gene_id(conn, gene_id, limit=200000)
            if types:
                rows = [r for r in rows if r["type"] in types]
        else:
            chrom, start, end = _parse_region_args()
    ...
    for r in rows:
        # Use the original attribute column verbatim (as stored at index time)
        # rather than reconstructing it from a handful of parsed keys, so
        # every exported line is byte-faithful to the source GTF (keeps
        # exon_number, protein_id, ccds_id, tag, etc. that we don't otherwise
...
def api_multiomics_index():
    ...
    if MULTIOMICS_DIR.is_dir():
        ...
        for cat_dir in cat_dirs:
            ...
            files = _sort_multiomics_files(files)
...
def api_multiomics_summary():
    ...
    # Atlas metadata is effectively static; cache for a day so repeated
    # navigations back to the home page never re-fetch this 275 KB payload.
    resp.headers["Cache-Control"] = "public, max-age=86400"
    return resp
...
def serve_multiomics_bw(category: str, filename: str):
    ...
    if not p.is_file():
        abort(404)
    # `.bw` content is immutable once generated, so let the browser cache byte
    # ranges aggressively. Without an explicit max_age, Flask emits
    # `Cache-Control: no-cache`, forcing igv.js to re-validate (a round-trip to
    # the slow shared disk) on every pan/zoom/re-toggle. `conditional=True`
    # keeps Range/ETag/Last-Modified handling intact.
    resp = send_file(
        p,
...
def api_download_resolve_region():
    ...
    chrom, pos : integers (position; resolves to single bp, extend with Step 2)
    ...
    if rtype == "position":
        ...
        try:
            ...
            abort(400, description="pos must be an integer")
        if pos < 1:
...
# ── BLAST async job pipeline ────────────────────────────────────────────────
#
# Mirrors the pyGenomeTracks pipeline above: BLAST runs as a subprocess that
# may take a few seconds, so jobs are queued on a small thread pool and the
# front-end polls /api/blast/status/<id> until done. Each job owns a directory
# under tmp/blast_jobs/<id>/ holding query.fa, out.json, run.log and status.json
# so any gunicorn worker can serve polling/result requests.

BLAST_JOBS_DIR = BASE_DIR / "tmp" / "blast_jobs"
BLAST_JOBS_DIR.mkdir(parents=True, exist_ok=True)
BLAST_JOB_TTL_SECONDS = int(os.environ.get("BLAST_JOB_TTL_SECONDS", str(24 * 3600)))
BLAST_CLEANUP_INTERVAL_SECONDS = int(
    os.environ.get("BLAST_CLEANUP_INTERVAL_SECONDS", str(24 * 3600))
)
BLAST_CLEANUP_LOCK = BLAST_JOBS_DIR / ".cleanup.lock"

_blast_executor = _cf.ThreadPoolExecutor(max_workers=2, thread_name_prefix="blast")
_blast_jobs_lock = _threading.Lock()
_blast_jobs: dict[str, dict] = {}


def _blast_cleanup_old_jobs() -> int:
    if not BLAST_JOBS_DIR.is_dir():
        return 0
    cutoff = time.time() - BLAST_JOB_TTL_SECONDS
    removed = 0
    for child in BLAST_JOBS_DIR.iterdir():
        try:
            if child.is_dir() and child.stat().st_mtime < cutoff:
                _shutil.rmtree(child, ignore_errors=True)
                removed += 1
...
def _blast_try_cleanup_old_jobs() -> None:
    BLAST_JOBS_DIR.mkdir(parents=True, exist_ok=True)
    try:
        with open(BLAST_CLEANUP_LOCK, "w", encoding="utf-8") as lock_f:
            try:
                import fcntl
                ...
                return
            n = _blast_cleanup_old_jobs()
            if n:
                app.logger.info(
                    "BLAST cleanup: removed %d expired job(s) from %s",
                    n, BLAST_JOBS_DIR,
                )
    except OSError:
        pass
...
def _blast_cleanup_loop() -> None:
    while True:
        try:
            _blast_try_cleanup_old_jobs()
        except Exception:  # pragma: no cover - defensive
            app.logger.exception("BLAST periodic cleanup failed")
        time.sleep(BLAST_CLEANUP_INTERVAL_SECONDS)


def _blast_start_cleanup_thread() -> None:
    t = _threading.Thread(
        target=_blast_cleanup_loop, name="blast-cleanup", daemon=True,
    )
    t.start()


_blast_try_cleanup_old_jobs()
_blast_start_cleanup_thread()


def _blast_status_path(job_id: str) -> Path:
    return BLAST_JOBS_DIR / job_id / "status.json"


def _blast_write_status(job_id: str, data: dict) -> None:
    job_dir = BLAST_JOBS_DIR / job_id
    job_dir.mkdir(parents=True, exist_ok=True)
    tmp = job_dir / "status.json.tmp"
    tmp.write_text(json.dumps(data), encoding="utf-8")
    os.replace(tmp, _blast_status_path(job_id))


def _blast_read_status(job_id: str) -> dict | None:
    p = _blast_status_path(job_id)
    if not p.is_file():
        return None
    try:
        return json.loads(p.read_text(encoding="utf-8"))
...
def _blast_lookup_job(job_id: str) -> dict | None:
    with _blast_jobs_lock:
        job = _blast_jobs.get(job_id)
    if job is not None:
        return job
    return _blast_read_status(job_id)


def _blast_xref_maps() -> tuple[set, set, dict]:
    """Return (annotated_ids, genome_perv_names, homo_coord_map) for cross-links.

    - annotated_ids   : sids with ORF/structure annotation (Browser has these).
    - genome_perv_names: PERVs placed directly on the reference (?perv= works).
    - homo_coord_map   : sid -> {chrom,start,end} for homologously mapped PERVs
                         (linked into the genome browser via ?loc=).
    """
    annotated = set(SEQ_OFFSETS.keys())
    try:
        perv_names = {s["name"] for s in _load_perv_list()}
    ...
    try:
        ...
        for s in seqs:
            qn = s.get("q_name")
            if (qn and qn not in homo and s.get("chrom")
                    and s.get("start") is not None and s.get("end") is not None):
                homo[qn] = {"chrom": s["chrom"], "start": s["start"], "end": s["end"]}
    except Exception:  # noqa: BLE001
        pass
    return annotated, perv_names, homo


def _blast_enrich_xref(parsed: dict) -> None:
    """Annotate each hit with which cross-links are valid for that PERV id."""
    annotated, perv_names, homo = _blast_xref_maps()
    for q in parsed.get("queries", []):
        for hit in q.get("hits", []):
            sid = hit.get("perv_sid") or ""
            hit["annotated"] = sid in annotated
            if sid in perv_names:
                hit["genome"] = {"mode": "perv"}
            elif sid in homo:
                h = homo[sid]
                hit["genome"] = {
                    "mode": "loc",
...
def _blast_run_job(job_id: str, spec: "_blast.BlastSpec") -> None:
    """Worker task: run BLAST and persist job state + parsed result to disk."""
    job_dir = BLAST_JOBS_DIR / job_id
    base = {
    ...
    try:
        running = {**base, "state": "running"}
        _blast_write_status(job_id, running)
        with _blast_jobs_lock:
            _blast_jobs[job_id] = running

        result = _blast.render(spec, job_dir)
        _blast_enrich_xref(result.parsed)
        (job_dir / "result.json").write_text(
            json.dumps(result.parsed), encoding="utf-8"
        )

        done = {**base, "state": "done", "finished": time.time()}
        _blast_write_status(job_id, done)
        with _blast_jobs_lock:
            _blast_jobs[job_id] = done
    except _blast.BlastError as exc:
        err = {**base, "state": "error", "error": str(exc), "finished": time.time()}
        _blast_write_status(job_id, err)
        with _blast_jobs_lock:
            _blast_jobs[job_id] = err
    except Exception as exc:  # pragma: no cover - defensive
        app.logger.exception("BLAST job %s crashed", job_id)
        err = {
            **base, "state": "error",
            "error": f"Internal error: {exc}", "finished": time.time(),
        }
...
@app.route("/api/blast/dbs")
def api_blast_dbs():
    """Return available BLAST programs + databases (with readiness flags)."""
    programs = [
        {
            "key": key,
            "query": info["query"],
        ...
        for key, info in _blast.PROGRAMS.items()
    ]
    databases = [
        {
            "key": key,
            ...
            "label_en": info["label_en"],
            "label_zh": info["label_zh"],
            "ready": _blast.db_ready(key),
        }
        for key, info in _blast.DATABASES.items()
    ]
    return jsonify({
        "programs": programs,
        "databases": databases,
        "evalue_choices": list(_blast.EVALUE_CHOICES),
        "max_target_default": _blast.MAX_TARGET_DEFAULT,
        "max_target_min": _blast.MAX_TARGET_MIN,
        "max_target_max": _blast.MAX_TARGET_MAX,
    })


@app.route("/api/blast/submit", methods=["POST"])
def api_blast_submit():
    """Queue a BLAST job. Returns ``{job_id}`` on success."""
    body = request.get_json(force=True, silent=True) or {}
    try:
        spec = _blast.validate_spec(body)
    except _blast.BlastError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception as exc:  # noqa: BLE001
        app.logger.exception("BLAST submit validation crashed")
        return jsonify({"error": f"Invalid request: {exc}"}), 400
    ...
    _blast_write_status(job_id, queued)
    with _blast_jobs_lock:
        _blast_jobs[job_id] = queued
    _blast_executor.submit(_blast_run_job, job_id, spec)
    return jsonify({"job_id": job_id})


@app.route("/api/blast/status/<job_id>")
def api_blast_status(job_id: str):
    if not re.fullmatch(r"[0-9a-f]{8,32}", job_id):
        abort(400, description="invalid job_id")
    job = _blast_lookup_job(job_id)
    if not job:
        return jsonify({"state": "unknown"}), 404
    return jsonify({
        "state":     job.get("state"),
        ...
        "db":        job.get("db"),
        "n_seqs":    job.get("n_seqs"),
        "submitted": job.get("submitted"),
        "finished":  job.get("finished"),
    })


@app.route("/api/blast/result/<job_id>")
def api_blast_result(job_id: str):
    if not re.fullmatch(r"[0-9a-f]{8,32}", job_id):
        abort(400, description="invalid job_id")
    job = _blast_lookup_job(job_id)
    if not job:
    ...
    if not result_file.is_file():
        abort(410, description="result has been cleaned up")
    return Response(result_file.read_text(encoding="utf-8"),
